...
function generateCalendar(date) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        if (diffInDays > 3) {
            dayBlock.classList.add("locked"); // Add a class to disable clicking
        } else {
            dayBlock.classList.add("clickable"); // Add a class for clickable dates
        }
...
calendar.addEventListener("click", function (e) {
    if (e.target.classList.contains("clickable")) {
        ...
        if (eventDates.includes(selectedDate)) {
            ...
            window.location.href = `/mini project/event-details/public/display.html?date=${selectedDate}`;
        } else {
            // No event -> Open `index.html` to create a new event
            window.location.href = `/mini project/event-details/public/index.html?date=${selectedDate}`;
        }
    }
});
